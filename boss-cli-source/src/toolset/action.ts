...
export async function runChatActionOnCurrentConversation(
  ...
  switch (action) {
    ...
    case 'remark':
      throw new Error('直接更新备注已禁用，请使用 recruitctl remark.update。');
    case 'resume': {
    ...
    case 'agree-resume':
      throw new Error('直接接收附件已禁用，请使用 recruitctl attachment.accept。');
    case 'request-attachment-resume':
      throw new Error('直接索取附件已禁用，请使用 recruitctl attachment.request。');
    case 'exchange-wechat':
      throw new Error('直接交换联系方式已禁用，请使用 recruitctl contact.exchange。');
    case 'history':
