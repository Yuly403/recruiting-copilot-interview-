...
/** 平台安全校验、403 或不支持页：自动化必须停止并交还给用户。 */
export function isBossVerificationUrl(url: string): boolean {
  try {
    const u = new URL(url);
    if (!u.hostname.includes('zhipin.com')) return false;
    return /\/web\/(?:common\/(?:403|nonsupport)\.html|user\/safe\/verify|passport\/(?:zp\/(?:403|verify|security)\.html|cm\/(?:403|verify|security-check)\.html))/i.test(u.pathname);
  } catch {
    return false;
  }
}
