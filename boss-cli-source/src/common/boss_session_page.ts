...
import { BOSS_CHAT_INDEX_URL, isBossChatShellUrl, isBossVerificationUrl } from './auth.js';
import {
...
function assertNotOnVerificationPage(page: Page): void {
  if (!isBossVerificationUrl(page.url())) return;
  const error = new Error('检测到 Boss 安全验证/403 页面，自动化已暂停；请在浏览器中人工完成验证后重试。') as NodeJS.ErrnoException;
...
export async function withBossSessionPage<T>(
  ...
  return withBossSessionLock(async () => {
    ...
    try {
      ...
      assertNotOnVerificationPage(page);
