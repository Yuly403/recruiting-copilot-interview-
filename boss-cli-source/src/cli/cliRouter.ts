...
用法与说明:
  ...
  boss action <resume|history>
      只读操作：查看在线简历或沟通历史
      not-fit 必须由招聘人员在浏览器中手工完成；其余写操作统一使用 recruitctl 审批链路
  boss send ...
      已禁用。请使用 recruitctl message.commit --plan ... --payload-file ...
  boss positions
  ...
  boss greet <姓名> [--job <岗位关键字>]
      已禁用。请使用 recruitctl greeting.commit --plan ... --payload-file ...
...
export async function executeCommand(argv: string[]): Promise<string> {
  ...
  if (cmd === 'action') {
    ...
    if (action === 'not-fit') {
      die('❌ 标记不合适不可自动执行，请由招聘人员在浏览器中核对候选人后手工完成。');
    }
    if (action !== 'resume' && action !== 'history') {
      die('❌ Boss 直接写操作已禁用。请使用 recruitctl 的 ActionPlan + 交互审批链路。');
    }
    return implChatAction({ action, remark });
  }

  if (cmd === 'send') {
    die('❌ Boss 直接发信已禁用。请使用 recruitctl message.commit --plan <计划> --payload-file <消息文件>。');
  }
  ...
  if (cmd === 'greet') {
    die('❌ Boss 直接打招呼已禁用。请使用 recruitctl greeting.commit --plan <计划> --payload-file <消息文件>。');
  }
