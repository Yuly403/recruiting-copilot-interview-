...
name: recruit-init
description: >
  初始化招聘工作区：检查 recruitctl (Gateway) / liepin-cli / wuyou-cli / lark-cli 前置依赖，创建标准目录骨架
...
```powershell
$env:LIEPIN_CLI_SOURCE = '<已审核的固定版本或本地源码路径>'
powershell -ExecutionPolicy Bypass -File skills/recruit-init/scripts/install-dependencies.ps1
```

脚本会检查 Node.js，优先从本仓库内已审计的 `boss-cli-source`、`recruiting-gateway`、`wuyou-cli`
构建并安装，在 macOS 等环境中自动修复 npm 全局命令的 `PATH`。猎聘源码未包含在仓库中，
因此脚本不再静默安装浮动最新版；运行前必须把 `LIEPIN_CLI_SOURCE` 设置为组织已审核的固定版本或本地源码路径。
...
2. `recruitctl help` —— 招聘网关 CLI（Gateway，统一入口）。没有 → 重跑安装脚本。
   不要直接用 `npm install -g git+...`：部分 npm 版本会把它留成指向临时缓存的符号链接。
   本脚本会先构建并打包 fork，再安装持久化的包文件。
3. `boss help` —— Boss 直聘 Legacy CLI（Gateway 的后端驱动器，不直接调用）。
   安装脚本会一并装好。没有 → 重跑安装脚本。
4. `liepin --version` —— 猎聘 CLI。没有 → 先确认并设置审核过的 `LIEPIN_CLI_SOURCE`，再重跑同一安装脚本。
5. `wuyou help` —— 前程无忧 CLI。没有 → 重跑同一安装脚本。
6. 本机装有 Chrome 或 Edge（三个 CLI 都靠它驱动真实浏览器）。
7. **可选**：`lark-cli --version` —— 有且已配置飞书应用凭证 → 日报出飞书云文档、
...
装好 CLI 后提醒用户各跑一次 `recruitctl session.login`、`liepin login` 和 `wuyou login`（扫码登录，登录态持久化）。
验证 Gateway 就绪：`recruitctl doctor` 应输出全部通过。如果用户此刻登录不了（比如手机不在身边），记入收尾提醒，继续建仓。
