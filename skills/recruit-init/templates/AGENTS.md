...
| 本地简历（猎头/内推/直投）要评估，或收取飞书邮箱近期猎聘/BOSS/前程无忧简历附件，或回查某人评级 | `skills/resume-review/SKILL.md` |
...
| boss 直聘操作 | **全部通过 `recruitctl` Gateway**（`recruitctl candidates.search / recommend / listUnread / preview`、`recruitctl greeting.commit`（招呼）等）。**禁止直接调用 `boss` CLI**——Gateway 提供幂等、限流、审计和 dry-run 模式。命令细节见 `skills/recruit-daily/references/channels.md` + `recruitctl help` |
| 猎聘 CLI 命令细节 | `skills/recruit-daily/references/channels.md` + `liepin help` |
| 前程无忧 CLI 命令细节 | `skills/recruit-daily/references/channels.md` + `wuyou help` |
...
**固定纪律：所有 BOSS 直聘操作必须走 `recruitctl` Gateway**——禁止直接调用 `boss` CLI。Gateway 提供幂等、限流、审计和 `--plan`（dry-run）模式；直接绕过去调用 `boss` 会丢失审计追踪、绕过限流保护并可能触发平台风控。猎聘和前程无忧继续使用各自的 CLI（`liepin`、`wuyou`）。
...
1. 新候选人先按不可逆的 `来源记录键` 精确去重，再以稳定的 `候选人键` append 进 `02-sourcing/dedup-ledger.csv`，定状态、记原因。姓名+岗位只能提示疑似重复，不能自动合并。
